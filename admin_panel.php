...
function getBookings($con) {
    $query = "SELECT b.id, b.booking_date, b.time_slot, b.status, f.name AS facility_name, u.username 
              FROM bookings b 
...
function isTimeSlotAvailable($con, $facility_id, $booking_date, $time_slot) {
    $query = "SELECT COUNT(*) as count FROM bookings 
              WHERE facility_id = ? AND booking_date = ? AND time_slot = ? AND status != 'cancelled'";
    $stmt = mysqli_prepare($con, $query);
    mysqli_stmt_bind_param($stmt, "iss", $facility_id, $booking_date, $time_slot);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_facility'])) {
        ...
        $query = "INSERT INTO facilities (name, hourly_rate, image_url) VALUES (?, ?, ?)";
        $stmt = mysqli_prepare($con, $query);
        mysqli_stmt_bind_param($stmt, "sds", $name, $hourly_rate, $image_url);
        mysqli_stmt_execute($stmt);
        ...
        $hourly_rate = floatval($_POST['hourly_rate']);
        $image_url = mysqli_real_escape_string($con, $_POST['image_url']);
        $query = "UPDATE facilities SET name = ?, hourly_rate = ?, image_url = ? WHERE id = ?";
        $stmt = mysqli_prepare($con, $query);
        mysqli_stmt_bind_param($stmt, "sdsi", $name, $hourly_rate, $image_url, $facility_id);
        mysqli_stmt_execute($stmt);
        ...
        if (isTimeSlotAvailable($con, $facility_id, $booking_date, $time_slot)) {
            $query = "INSERT INTO bookings (user_id, facility_id, booking_date, time_slot, status) VALUES (?, ?, ?, ?, 'confirmed')";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, "iiss", $user_id, $facility_id, $booking_date, $time_slot);
            mysqli_stmt_execute($stmt);
            $success_message = "Booking added successfully.";
        } else {
            $error_message = "The selected time slot is not available. Please choose another.";
        }
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden p-6">
                ...
                <form action="" method="post" class="mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
                        ...
                        <select name="facility_id" required class="p-2 border rounded">
                            <option value="">Select Facility</option>
                            <?php foreach ($facilities as $facility): ?>
                                <option value="<?php echo $facility['id']; ?>"><?php echo $facility['name']; ?></option>
                            <?php endforeach; ?>
                        </select>
                        <input type="date" name="booking_date" required class="p-2 border rounded">
                        <select name="time_slot" required class="p-2 border rounded">
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="py-2 px-4"><?php echo date("g:i A", strtotime($booking['time_slot'])); ?></td>
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden p-6">
                ...
                <form action="" method="post" class="mb-6">
                    <div class="flex gap-4">
                        ...
                        <input type="text" name="image_url" placeholder="Image URL" required class="flex-grow p-2 border rounded">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-4">
                        <form action="" method="post" class="flex-grow flex gap-4">
                            ...
                            <input type="text" name="image_url" value="<?php echo $facility['image_url']; ?>" required class="flex-grow p-2 border rounded">
